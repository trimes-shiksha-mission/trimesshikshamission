...
export const Blog: FC<{ type: string }> = ({ type }) => {
  ...
  const [previewModal, setPreviewModal] = useState<RouterOutputs['blogs']['getAll']['blogs'][0] | null>(null)
  const [variables, setVariables] = useState({
    page: 1,
    limit: 10
  })

  const { data: blogs, isLoading: getBlogsLoading, refetch } = api.blogs.getAll.useQuery({
    type,
    ...variables
  })

  const { mutateAsync: createBlog, isLoading: createBlogLoading } = api.blogs.createOne.useMutation()

  return <div>

    <Button icon={<FileAddOutlined />} className="mb-2" onClick={() => setAddBlogModal(true)}>Create</Button>
      ...
      columns={[
        ...
        },
        {
          ...
          render: (_, record) => <Space>
            <Button onClick={() => setPreviewModal(record)} icon={<EyeOutlined />} />
